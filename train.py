...
############load preTrained model to continue train##############
model_file = './models/saved_model3/faster_rcnn_59400.h5'
network.load_net(model_file, net)
############end load preTrained model to cintinue train#############
...
optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, params[8:]), lr=lr, momentum=momentum, weight_decay=weight_decay)
...
for step in range(start_step, end_step+1):
    try:
        ...
        if step % disp_interval == 0:
            ...
            if _DEBUG:
                log_print('\tTP: %.2f%%, TF: %.2f%%, fg/bg=(%d/%d)' % (tp/fg.item()*100., tf/bg.item()*100., fg/step_cnt, bg/step_cnt))
                log_print('\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box: %.4f' % (
                    net.rpn.cross_entropy.data.cpu().numpy().item(), net.rpn.loss_box.data.cpu().numpy().item(),
                    net.cross_entropy.data.cpu().numpy().item(), net.loss_box.data.cpu().numpy().item())
                )
            re_cnt = True
    ...
    try:
        ...
        del im_data
        del im_info
        del gt_boxes, gt_ishard, dontcare_areas, loss
    except:
        print('del fail')
    for obj in gc.get_objects():
        try:
